...
public sealed class FolderConformanceTests : IDisposable
{
	...
	public async Task FolderCreate_CalendarType_RoutesToTheCalendarStore()
	{
		...
		calendar.Listing = [new BackendFolder { Key = new FolderKey("caldav:MyCalendar"), DisplayName = "MyCalendar", Type = FolderType.UserCalendar }];
	...
	public async Task FolderCreate_MailType_StillCreatesInTheMailStore()
	{
		...
		_harness.Session.Store.Listing = [new BackendFolder { Key = new FolderKey("imap:Projects"), DisplayName = "Projects", Type = FolderType.UserMail }];
	...
	public async Task FolderUpdate_WithADifferentParentId_IsRejected_NotSilentlyRenamedInPlace()
	{
		List<UserFolder> registry = await _harness.RegisterFoldersAsync(
			EasHandlerHarness.Folder("imap:Parent", "Parent", FolderType.UserMail, EasClass.Email),
			EasHandlerHarness.Folder("imap:Child", "Child", FolderType.UserMail, EasClass.Email, "imap:Parent"));
