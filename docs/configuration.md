...
| `AutoProvisionUsers` | `true` | Whether an **undeclared** login may exist at all. `true`: a login that authenticates gets a user on first successful sign-in (a fresh immutable user id plus an auto-provisioned declaration), so it is visible/manageable (`eas users`, admin UI), blockable, and able to use the self-service portal — auth itself is unchanged (no gateway password, so it still probes). `false`: **allowlist mode** — an undeclared login is refused *before any backend probe*, so undeclared credentials never reach the mail server; declared logins (config or database; an empty entry `{}` is a valid grant) still work. **This key absorbed the removed `RequireDeclaredUsers`** — same name, sharper meaning. See [Database-declared users](../README.md#database-declared-users-eas-user-). |
...
| `CalendarAttachments` | `"Auto"` | Event attachments for EAS 16.x clients: `Auto` (enabled, 1 MiB per attachment), `On` (enabled, 16 MiB) or `Off`. Attachments are stored **inline** in the event (base64 `ATTACH` property), so they work against any calendar backend — the cap is what keeps inline blobs from bloating the stored items. A HOST key: the cap governs what the gateway's own EAS merge writes into the iCalendar. *(Moved here from `Backends:Calendar:CalendarAttachments`, where it was per-user overridable; it is now global.)* |
...
| `SendDedupRetentionDays` | `30` | Days a completed send-dedup claim (send-by-reference, meeting response) is kept before a background sweep deletes it. These two claim kinds are never reclaimed by a Sync collection commit, unlike every other dedup claim; `0` disables the sweep. |
