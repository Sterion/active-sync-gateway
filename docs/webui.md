...
- A **disabled** account (`eas user disable`, or the Enabled toggle / Disable button on the
  ...
  device — until re-enabled. That is the whole-user switch; a `block` is per-device, so it has
  no bearing on a web session (there is no device in one). The CLI remains the un-lockable
  escape hatch.
...
| `GET/PUT/DELETE /admin/api/users[/{login}]` | Declared users with per-field provenance (each value tagged with the level that supplied it); PUT = full-replacement of the DATABASE DECLARATION with password sentinels (null = keep, `""` = clear, value = hash/seal) plus the `enabled` flag, validated like the CLI. Keep-sentinels resolve against the effective merged entry — what the GET showed you masked. DELETE drops the declaration only, so its fields fall back to configuration; deleting the USER is the separate confirm-and-cascade route below. PUT answers `{ user, warning }`. A write that would leave **no enabled administrator** is refused 409 — recovery from zero admins is CLI-only, so the UI does not let you walk into it; editing your OWN account down is allowed but warned about. |
| `POST /admin/api/users/{login}/disable·enable` | Flip the user master switch without a full-replacement PUT; a disabled user refuses every login (403). Same `{ user, warning }` shape and the same last-administrator 409. |
| `POST /admin/api/users/{login}/rename` | Change a user's login (`{ newLogin }`). A single-row update — sync state, devices and locally-stored items are unaffected. Refused 400 for a login declared in configuration (change it there; a config-declared login is immutable so the two sides cannot drift) and for a target already taken by another user or by a config entry. |
| `GET /admin/api/users/{login}/deletion-impact` · `POST .../delete` | Confirm-and-cascade user deletion, distinct from the DELETE above (which only drops the database declaration). GET reports what would be destroyed — content per collection vs sync state — without touching anything; POST requires the login echoed in `confirm`, and the refusal names the loss when locally-stored items are at stake. |
...
| `GET /admin/api/devices` · `POST .../block·unblock·wipe·purge` | Device management; wipe/purge require the target echoed back in `confirm`. **Blocks are per-device**: `deviceId` is required and a request without one is refused 400 pointing at disabling the user instead. GET is paged: `{ total, entries }`, `limit` (default 200, max 500), `offset` and an optional `user` filter; each entry reports `blocked` (this device) and `userDisabled` (the user) separately. |
