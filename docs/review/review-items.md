...
**11. CLI auth & envelope hardening** — ~~`K7`~~ ~~`K16`~~ ~~`E3`~~ ~~`E8`~~ **COMPLETE**
...
> **Resolution:** `K7` `TryAuthorize` now refuses `device password` / `user secret` outright when no master key is configured (AllowPlaintext mode), instead of executing them and returning the credential unsealed — folded into the auth decision (not surfaced as a command result) so the refusal stays a request-level 404, keeping the `eas` client's existing "404 ⇒ nothing ran ⇒ safe to fall back to local execution" contract intact; the command still runs, just locally instead of over the wire. Proven red-first (`Authorize_RefusesCredentialBearingVerbs_InAllowPlaintextMode`). `K16` `LocalCliEnvelope.TryOpen` gained an optional `seenNonces` set so a single call can be self-enforcing for single-use, independent of a caller's own replay bookkeeping (default omitted preserves prior behavior). N/A for red-first — `LocalCliEndpoint`'s existing `ReplayCache` already enforced single-use correctly; this hardens the API contract for future callers, there was no defective behavior to reproduce. `E3` `RunCapturedAsync` now shares ONE `TextWriter.Synchronized` instance between the Console-routing path and `AnsiConsole`'s `Out` (previously two independent wrappers over the same `StringWriter`, which still raced since `Synchronized` locks on itself, not the wrapped writer). COVERAGE, not red-first proof — the wiring is a private local and no command in the tree fans out concurrent writes, so the race can't be triggered end-to-end through `ExecuteAsync`; two tests reproduce the identical wiring pattern standalone (unfixed shape corrupts under load, fixed shape does not). `E8` `ProtectAsync` now zeroes the master key in a `finally`, covering the empty-secret early return the successful-seal-only zero missed. COVERAGE (mirrors the L42 precedent) — the key is a local `byte[]` with no external handle, so zeroing isn't observable; the test only proves the early-return path still behaves correctly under the new `try/finally`.

**12. SSRF, oracle & info disclosure** — ~~`C2`~~ ~~`C5`~~ **N/A** ~~`E6`~~ ~~`E11`~~ **COMPLETE**
> **C5 closed N/A by human decision** — the disclosure is real but the remedy is disproportionate, and no
> code change was kept. Full reasoning on the finding below and in `review-results.md` § Item 12. Short
> version: the portal caller can already OVERWRITE this field, and showing it to them is what stops them
> destroying their own sync; closing it properly would need per-field provenance on `BackendRoleOverride`
> — a config-bindable contract type — plus a clear-on-every-admin-write invariant that can silently drift.
> That is a poor trade for a Low disclosure. The two attempts already made (item 12's gate `7f0c73b`, its
> revert `fec1cfe`) both stand reverted; do not re-apply a settings-surface gate.
...
**19. Backend session lifetime & auth cache** — ~~`A5`~~ ~~`A6`~~ ~~`A8`~~ ~~`A9`~~ ~~`A10`~~ **COMPLETE**
...
**25. Compose, move & folder handlers** [LIVE] — ~~`F2`~~ ~~`F4`~~ ~~`F5`~~ ~~`F6`~~ **COMPLETE**
...
`A2` **Med** Batched DAV-id insert loses this request's new ids when a concurrent unique-violation rolls back the batch — `State/DavItemMap.cs:97`. FIXED (item 20): `GetOrAddBatchAsync` now loops (bounded, `MaxBatchAttempts = 5`) instead of doing a single re-read-and-return after a unique violation — each pass re-stages whatever hrefs are STILL missing from the re-read and retries, so a href new to this request (and not created by the racer) is no longer silently dropped from the returned map. The last attempt's exception propagates unswallowed if contention never clears. Proven red-first (`GetOrAddDavItemIds_ConcurrentUniqueViolation_DoesNotDropThisRequestsNewHrefs`, via a `FaultInjectingInterceptor` that throws a real-shaped unique-violation on the first save with no actual competing writer — isolating exactly the "re-read finds nothing" case the finding describes).
`A3` **Med** `SaveOofAsync`/`GetOofAsync` have no concurrency token / insert-race guard — `State/SyncStateService.cs:185`. FIXED (item 20): took the "catch `IsUniqueViolation` and re-read like DeviceStore/DavItemMap" option from the finding's own text rather than adding a concurrency token (which would need a migration on both providers for a Low/Med-severity race). `SaveOofAsync` now tracks whether it is inserting; on a unique-violation `DbUpdateException` from that insert it detaches the losing row, re-reads the winner, re-applies this call's fields as an update, and saves again — mirroring `DeviceStore.GetOrCreateDeviceAsync`'s existing idiom for the identical race shape. The read-modify-write lost-update case between two concurrent UPDATEs (as opposed to the INSERT race) has no conflict detection either way, before or after — not addressed by this fix, consistent with the finding's narrower "insert-race guard" framing and the DeviceStore precedent it points at. Proven red-first (`SaveOof_ConcurrentInsertRace_UpsertsInsteadOfThrowing`), using a `ConcurrentWriteInterceptor` that inserts and commits a REAL competing row via a separate context over the same connection right as the save fires — the constraint violation and the recovery's re-read are both genuine, not simulated.
...
`A5` **Med** Auth success cache admits a stale password after a backend-side change — `Backend/BackendSessionFactory.cs:96`. FIXED (item 19): the finding itself calls this "bounded and arguably a documented tradeoff" — a "no positive cache" mode already exists (`SuccessCacheMinutes=0`, already validated 0-1440) and `docs/configuration.md` already stated the staleness window. What was missing was the SAME caveat in the operator-facing option help (`SettingKeys.cs`, surfaced by `eas config` and the web admin Settings page), which said only "Good-credential cache TTL (0 disables)" with no hint of the tradeoff. Its `Description` now states the window explicitly: "A password revoked or changed on the mail backend keeps working against the gateway for up to this long (0 disables — every request re-verifies)." N/A for red-first — no code behavior changed (the 0-disables path already existed and is already covered by `ActiveSyncOptionsValidatorTests`), so there is no defect symptom to reproduce; this is a documentation-parity fix, not a behavior fix.
`A6` **Low** `CompleteAccountWipeAsync` can 500 on a concurrency conflict during a wipe ack — `State/DeviceStore.cs:165`. FIXED (item 19): the save loop now catches `DbUpdateConcurrencyException` (bounded to 4 attempts) alongside the existing `LoginBlock` unique-violation catch — on a concurrent `Device` write it reloads the row and re-applies `PendingAccountWipe = false` on the next pass instead of letting the exception surface as a 500. Proven red-first (`CompleteAccountWipe_ConcurrentDeviceConflict_DoesNotThrow`).
`A7` **Low** `SnapshotCodec.Decompress` uses a default-comparer dict while the diff engine uses Ordinal — `State/SnapshotCodec.cs:36`. FIXED (item 20): both the empty-blob short-circuit and the deserialized-map path now construct explicitly with `StringComparer.Ordinal` (the deserialize target is re-wrapped via `new Dictionary<string,string>(deserialized, StringComparer.Ordinal)`, since `JsonSerializer.Deserialize` itself has no comparer parameter), matching FolderRegistry/DavItemMap. BEHAVIOUR NOTE: no observable lookup/hash difference today — .NET's default string equality comparer is already ordinal — so this is a structural/defensive fix, not a bugfix with a reproducible symptom. Proven red-first on the ACTUAL comparer object (not lookup behaviour, which cannot differ): `Decompress_UsesOrdinalComparer_MatchingTheDiffEngine` asserts `result.Comparer` is reference-equal to `StringComparer.Ordinal`, which fails against the unmodified default-comparer dictionary and passes after.
`A8` **Low** Cache-clear on snapshot change races an in-flight verdict write (TOCTOU) — `Backend/BackendSessionFactory.cs:62`. FIXED (item 19): `AccountResolver` now exposes a monotonic `SnapshotVersion`, bumped (`Interlocked.Increment`) on every rebuild (both `EnsureFreshAsync`'s stamp-changed path and `OnRolesChanged`), alongside the existing `SnapshotChanged` clear. `BackendSessionFactory.AuthenticateAsync` captures the version live at decision time and stamps it onto every `_authCache`/`_authNegativeCache` entry; a cache hit is honoured only when its stamped version still equals the CURRENT version. A verdict computed against a superseded snapshot that writes back after the clear (the TOCTOU) is therefore tagged stale and rejected on the next read, instead of being trusted. Proven red-first (`AuthCache_RejectsAVerdictStampedWithAnOlderSnapshotVersion`, driving a real in-flight probe past a concurrent DB-triggered rebuild via a controllable `ICredentialVerifier`).
`A9` **Low** `activeUsers` counts faulted/unrealized Lazy slots, so watchers aren't trimmed — `Backend/BackendSessionFactory.cs:315`. FIXED (item 19): `EvictIdleSessionsCore` now filters `_sessions` to `IsBuilt(pair.Value)` before deriving `activeUsers`, matching the `SnapshotSessions`/`GatewayMetrics` observer convention already used elsewhere in the class — a still-building, never-realized, or faulted (A10) slot no longer pins its user's `IPerUserResourceOwner` trim. Proven red-first (`IdleSweep_DoesNotCountAFaultedSessionSlotAsActive`).
`A10` **Low** Faulted Lazy session slots never swept; wedge a (user,device) into repeated failures — `Backend/BackendSessionFactory.cs:296`. FIXED (item 19): `GetSessionAsync` now catches the faulted-build exception around `AwaitBuild`, value-compared `TryRemove`s the faulted slot, and rebuilds exactly once (bounded by a `retriedFault` flag, so a genuinely-down backend still fails the call with its real exception rather than retrying forever) — a transient outage now self-heals within the same call instead of wedging the (user, device) until restart. `EvictIdleSessionsCore` also sweeps any faulted slot on its own (a new `IsFaulted` check alongside the existing `IsBuilt` idle-timeout branch), so a key that's never retried doesn't leak. Proven red-first (`GetSession_RecoversFromASingleFaultedBuild_WithoutWedgingTheSlot`, `IdleSweep_RemovesAFaultedSessionSlot`).
...
`B3` **Med** Startup-impact validation can't catch a backend/user-validator brick — `Administration/SettingKeys.cs:271`. FIXED (item 22): documentation-only, after confirming the gap is structural rather than a live reachable defect — every catalogue key in `SettingKeys.All` lives outside both the `ActiveSync:Backends` and `ActiveSync:Users` sections `BackendConfigurationValidator` reads, so a catalogue write's "effective ⊕ candidate" configuration is byte-identical in those sections before and after; wiring the registry-based validators into `ValidateStartupImpact` would compare a failure set against itself and could never surface anything — dead code that also costs a `BackendRolesConfig.Load` + full `AccountResolver.ValidateUsers` pass on every settings write. Documented the scope and the "why extending it would be a no-op today" reasoning directly on `ValidateStartupImpact`'s XML doc, including the "extend this if a future catalogue key ever reads into Backends/Users" trigger. N/A for red-first: no behavior changed.
`B4` **Med** `EnsureFreshAsync`/`OnRolesChanged` race on `_snapshot`; a stale refresh overwrites a newer one — `Accounts/AccountResolver.cs:117,170`. FIXED (item 22): took the "serialize both writes under one lock" option from the finding's own text. A new `_snapshotSwapLock` (plain `object`, `lock`) now wraps the BUILD (not just the final assignment) and swap in both `EnsureFreshAsync` and `OnRolesChanged` — whichever writer runs second always builds against whatever the first one just installed (current roles, current database users), so neither can clobber the other's newer snapshot with one computed from now-superseded inputs. Proven red-first (`ConcurrentRoleChange_DuringAccountRefresh_IsNotLostToTheStaleRefreshFinishingLast` in `AccountStoreTests.cs`) with a test-only provider whose `ValidateConfiguration` blocks once until released, assigned to the Notes role, opening a real, deterministic window inside `BuildSnapshot` between "roles captured" and "swap" — the same shape a slow CA read or a plugin's own validation creates in production. Unmodified code showed the exact symptom (final resolved settings = the stale roles, "v1", not the live "v2"); the fix makes it "v2" every time.
`B5` **Low** `Users` config secrets unsealed and retained in the long-lived snapshot — `Accounts/AccountResolver.cs:446`. FIXED (item 22): documentation-only — took the "or document the accepted residency" option. Re-deriving secrets lazily at `Resolve` time would need the master key reloaded (and potentially PBKDF2-stretched from a passphrase, K3) on every request that resolves an account with a sealed backend secret — a real per-request cost on the hot path, trading it for a memory-residency window that is already gated behind reading process memory with the master key present, for a Low-severity finding. Documented the tradeoff directly on `ResolveSecret`'s XML doc (bounded by `UsersRefreshSeconds`, not zeroed since a .NET `string` can't be scrubbed deterministically anyway). N/A for red-first: no behavior changed.
`B6` **Low** SQLite "missing table" detected by a localized message substring — `Settings/DbSettingsLoader.cs:72`.
`B7` **Low** Legacy `sieve.enabled` upgrade yields an un-authenticable Oof row with no Host — `Administration/LegacyAccountJson.cs:108`. FIXED (item 22): took the "drop the Oof override when no Host can be derived" option. `TryConvert` now only sets `oof.Provider = "sieve"` (and keeps the override at all) when the converted section actually carries a `Host` key; otherwise the Oof override is dropped entirely (the rest of the account still upgrades and authenticates normally) and a `LogWarning` names the actionable fix (`Backends:Oof:Settings:Host`). Proven red-first (`SieveOptIn_WithNoHost_DoesNotProduceAnUnauthenticableOofOverride`) — unmodified code produced a Backends entry `{Provider:"sieve"}` with no host for `{"sieve":{"enabled":true,"userName":"..."}}` (a bare `enabled:true` with nothing else already converts to no override at all, so the input needed one other field to exercise the bug).
...
`C2` **Med** Backend `/test` probe is an SSRF reachability oracle for arbitrary operator hosts — `Api/BackendsEndpoints.cs:178`. FIXED (item 12): the `/test` handler now builds the probed `ProviderSettings` from the role's currently-STORED settings only (`Merge(parsed, new RoleWriteRequest(request.Provider, null), db, config, ...)`) — a request-body `Settings` override (Host/Port/BaseUrl/...) is never fed to the probe. The provider CHOICE still travels from the request (selects among the fixed, already-registered provider set, not a network address). BEHAVIOUR CHANGE: an admin can no longer "test" an as-yet-unsaved Host/BaseUrl before saving it — only the persisted configuration is ever probed.
...
`C5` **Low — CLOSED N/A** Portal `me` echoes the admin-set backend username to a non-admin — `Api/PortalEndpoints.cs:48`. **RESOLVED N/A (human decision) — no code change kept. Do not re-open without new information; two attempts already failed.** THE DEFECT AS FILED: for a role whose credentials an admin configured (often a shared mail login or service account), the portal `me` response discloses that `userName` verbatim to the non-admin portal user, who cannot change Provider/Enabled. WHY IT IS CLOSED RATHER THAN FIXED, in order of weight: (1) **the same caller may already overwrite this field unconditionally** (`PUT /user/api/backends/{role}` sets `UserName` for any role with no `SelfServiceEditable` check, `PortalEndpoints.cs:233`), and if they do, THEIR OWN SYNC BREAKS until an admin repairs it — so echoing the value is what stops a user destroying a credential they can already destroy blind; withholding it makes the portal more dangerous, not less. (2) A correct fix requires distinguishing *the value an admin set* from *the value this user set*, and `BackendRoleOverride` (`Core/Options/AccountOptions.cs:9`) carries no per-field provenance. Adding it means a new property on a type that is BOTH config-bound (`ActiveSync:Users`, where a provenance flag is meaningless and settable) AND JSON-serialized into `AccountEntry`, plus a new clear-it-on-every-admin-write invariant (`eas user set`, admin API) that drifts silently the first time a write path forgets — a flag that fails open is worse than no flag, because it reads as closed. (3) Legacy rows have no provenance either way: fail-closed hides every pre-upgrade self-set username from its owner, fail-open keeps disclosing admin-set ones. No good default. (4) Severity is **Low**, and the value is not a secret — it is the login half of a pair whose password is already masked (`passwordSet` bool, never the value). FAILED ATTEMPTS, for anyone tempted: item 12 gated the echo on the *settings* self-service surface (`HasSelfServiceSurface`, `7f0c73b`) — wrong surface, since credentials are unconditionally self-editable; it broke the write/read round trip and failed two live integration tests, and was reverted (`fec1cfe`). **If this is ever re-opened**, the only sound fix is provenance (option 2 above) — never a settings-surface gate — and it should be weighed against simply masking the value in the UI while keeping it readable to its owner.
...
`E4` **Med** DB-log shutdown flush uncancellable, blocks up to 2 s with no save token — `Setup/DatabaseLogSink.cs:73,107`. FIXED (item 21): `Dispose` now cancels a `_shutdownCts` token (in addition to `_channel.Writer.TryComplete()`) before its bounded `_drain?.Wait(2s)`, and that token is threaded through into `DrainAsync`'s `WaitToReadAsync`/`SaveChangesAsync` calls — so an in-flight read/write is actually interrupted on shutdown instead of merely being abandoned once `Wait(2s)` times out. The inner per-batch catch re-throws a shutdown-token OCE (rather than logging it as a "batch write failed" fault) so the outer catch treats it as a clean exit. `Dispose` was made idempotent (`Interlocked.Exchange` guard) since Serilog's own `Logger.Dispose()` also disposes an `IDisposable` sink it owns, and a second `_shutdownCts.Cancel()`/`Dispose()` pair would otherwise throw `ObjectDisposedException`. Proven red-first: a `SaveChangesInterceptor` hangs on `Task.Delay(Timeout.InfiniteTimeSpan, ct)`; on unmodified code (no token reaches the interceptor) the hang is never cancelled even after `Dispose()` returns, on fixed code it cancels within Dispose's own window — `DatabaseLogSinkTests.Dispose_CancelsAnInFlightHungWrite_InsteadOfLeavingItUncancelled`.
...
`E6` **Med** `/readyz` treats a null peer as local, disclosing backend topology — `Setup/ReadinessResponse.cs:26`. FIXED (item 12): `IsLocal` now treats a null `RemoteIpAddress` as local ONLY under the test-host seam the integration suite already sets (`AS_TEST_FORCE_SERVE`, from `TestBootstrap.cs`'s module initializer) — everywhere else (including a real deployment where some transport legitimately delivers a null peer) it is non-local, so the component/topology detail stays withheld. A real loopback peer is unaffected. Proven red-first (`ReadinessResponseTests.IsLocal_NullPeer_IsNotLocal_OutsideTheTestHostSeam`).
...
`E11` **Low** Autodiscover body buffered to a string with no size cap — `Eas/AutodiscoverEndpoint.cs:95`. FIXED (item 12): `ExtractEmailAsync` now reads the request body in 4 KB chunks into a bounded buffer, bailing out (returns null, same as any other malformed request) once the total exceeds a 16 KB cap — checked against bytes actually read, not the `Content-Length` header, so a chunked-transfer body can't bypass it. Made `internal` (from `private`) for direct unit testing, matching `BuildEasUrl`'s existing pattern; no other behaviour change. Proven red-first (`AutodiscoverBodyCapTests.ExtractEmailAsync_OversizedBody_IsNotFullyBufferedOrParsed`).
`E12` **Low** CLI users-file error is a raw framework exception, unlike the server path — `Cli/CliVerbs.cs:39`.
`E13` **Low** DB-log drain spins tight when `Log:Database` is toggled off live — `Setup/DatabaseLogSink.cs:92`. FIXED (item 21): the disabled-live discard branch now awaits a 200 ms `DisabledDiscardPause` (cancellable via the E4 shutdown token) before looping back to `WaitToReadAsync`, instead of an immediate `continue` — bounds CPU to one discard pass per pause regardless of how fast the channel refills, while still draining a full batch (up to 500) each pause. Added a test-only `internal BufferedCount` accessor on the channel for observability. Proven red-first: pre-buffer 6,000 entries (before `Activate`, so they sit in the channel unconditionally), then `Activate` with `Log:Database=false` and time how long the backlog takes to fully drain — unmodified code drained all 6,000 in ~27 ms (the tight spin), fixed code takes the throttled ~2.4 s (12 batches × 200 ms) — `DatabaseLogSinkTests.DisabledLive_DiscardsBufferedBacklog_WithoutTightSpinning`.
...
`H2` **Med** CalDAV create always does a full pre-PUT enumeration (contradicts H13) — `Dav/DavStoreBase.cs:98`. FIXED (item 23), then found broken and corrected (scoped repair, post-item-23): item 23's original fix made `before` a memoized `Func<Task<...>>` invoked only inside `ResolveStoredHrefAsync`, which runs AFTER the PUT — so whenever it WAS invoked, it enumerated a collection that already contained the just-created resource, even though it was still named and documented as the listing "from before the PUT". That broke exactly the two paths the function exists for: a UID-query hit at a canonicalized (server-rewritten) href was wrongly rejected (`!before().ContainsKey(hit.Href)` was now always false, since the post-PUT listing does contain it), and the listing-diff fallback's `appeared` set was always empty (both "before" and "after" were the same post-PUT snapshot), falling through to the warning path and returning the wrong href. A genuine pre-PUT snapshot cannot be reconstructed after the PUT, so the repair does not restore an eager fetch — it removes the before/after diff entirely. `ResolveStoredHrefAsync` now verifies a UID-query hit at a non-PUT href by fetching its content and checking the embedded UID (`TryVerifyByContentAsync`) instead of a presence/absence proxy, and the deep fallback (no UID-query support, href rewritten) scans the post-PUT listing's candidates the same way. A well-behaved server (UID query resolves the exact PUT href) still returns after one REPORT call — no listing enumeration, no content fetch — so H2's original optimization holds. Proven red-first: the fast path stays covered by `CardDavStoreTests.CreateItem_WhenUidQueryMatchesPutHref_SkipsTheFullEnumeration` (0 PROPFINDs); the canonicalized-href case is covered by new test `CreateItem_WhenServerCanonicalizesHref_AdoptsTheCanonicalHref` (a stub server storing under a href different from the PUT target), which returned the wrong (PUT-target) href on the item-23 code and returns the correct canonical href after this repair.
`H3` **Med** JSCalendar duration off by an hour across DST (floating wall-clock subtraction) — `Jmap/JsCalendarConverter.cs:175`. FIXED (item 23): timed events now compute `end.AsUtc - start.AsUtc` (the UTC instants, honoring the event's TZID/DST) instead of subtracting the floating `.Value`s; all-day events are unchanged (`.Value - .Value`, already zone-free and exact). Proven red-first (`JsCalendarConverterTests.Duration_AcrossDstBoundary_IsComputedFromUtcInstants`, a Europe/Copenhagen event spanning the 2026-03-29 spring-forward): unmodified code produced `PT6H` (naive wall-clock diff), fixed code produces the correct `PT5H`.
...
`H8` **Low** JSCalendar recurrence `until` written as naive local though parsed as UTC — `Jmap/JsCalendarConverter.cs:274`. FIXED (item 23): `FromRecurrenceRule` now takes the event's `TzId` and a new `FormatUntil` helper converts the parsed UTC instant into that zone (`TimeZoneInfo.ConvertTimeFromUtc`) before formatting the local wall-clock string; a floating event or an unresolvable zone id falls back to the previous raw-UTC-digits behaviour. Proven red-first (`JsCalendarConverterTests.Recurrence_Until_IsWrittenInTheEventsOwnZone_NotRawUtc`, a Europe/Copenhagen weekly event with `UNTIL=20261001T070000Z`, still CEST/UTC+2 on that date): unmodified code wrote `2026-10-01T07:00:00` (the raw UTC digits), fixed code writes the correct local `2026-10-01T09:00:00`. Scope note: the finding's own FIX text targets only the format/write direction, which is what this changes; `ToRecurrenceRule`'s mirror-direction parse (JS `until` → iCal, `AssumeUniversal`) is unchanged, consistent with the finding's stated scope.
`H9` **Low** JMAP blob/eventsource requests don't re-assert same-origin before attaching credentials — `Jmap/JmapClient.cs:246`.
`H10` **Nit — CLOSED N/A** `CategoriesOfAsync` issues an extra serial `Email/get` before the category patch — `Jmap/JmapMailStore.cs:264`. **RESOLVED N/A (judgment call) — no code change kept.** The suggested fold ("get→set→get in one `InvokeAsync`") is not achievable while preserving correctness: JMAP's method-call `resultReference` (RFC 8620 §3.7; this file's own `ResultRef` helper, used at `:160` for `Email/query`→`Email/get`) only substitutes a LITERAL value/array copied from an earlier call's result — it cannot express "the set of currently-present custom keywords NOT in the wanted set," which is exactly what the categories patch needs to decide what to null. That computation has to happen in C# after reading the `Email/get` response, so the `Email/set` call's arguments cannot be built until that response is in hand — a genuine read-then-write-based-on-what-was-read dependency, which structurally requires two HTTP round trips whenever a Categories change is present, not a batching oversight. Verified there's no OTHER caller of `CategoriesOfAsync` to fold instead. The only way to actually remove the round trip would be for `SyncHandler` to hand `UpdateItemAsync` the categories it already knows from its own revision map (per the sync model's `|kw1,kw2` revision suffix) — an `IContentStore` interface change touching every backend's `UpdateItemAsync` signature, wildly disproportionate to a Nit. Filed as `H35` below for a future item if that interface change is ever undertaken for a stronger reason.
`H11` **Nit** `StripEmailDisplay` keeps display-name text as the address on a malformed input — `Jmap/JsContactConverter.cs:407`. FIXED (item 23): `StripEmailDisplay` now first tries `MimeKit.MailboxAddress.TryParse` (MimeKit already referenced transitively via Backends.Common and used directly elsewhere in this assembly) — its lenient parser recovers the intended address from a truncated `"Name <addr"` shape (a `<` with no matching `>`); the old substring-or-whole-string heuristic is kept only as a last resort for input with no recognizable address at all. Proven red-first (`JsContactConverterTests.FromApplicationData_MalformedAngleBracketEmail_ExtractsTheAddressPart`, `"Display Name <notanemail"`): unmodified code kept the entire string (including the display name and the stray `<`) as the JMAP `address` member, fixed code extracts `"notanemail"`.
...
`C10` **High** The admin Backends page's "Test connection" button silently probes something other than what
is on screen — introduced by `C2` (item 12) — `src/ActiveSync.WebUi/wwwroot/admin/views/backends.js:115`
(`testButton` → `collect()`) against `src/ActiveSync.WebUi/Api/BackendsEndpoints.cs:155` (the `/test`
handler). The SPA posts `collect()`, which is `{ provider, settings }` built from the **current on-screen
form values** — including unsaved edits. `C2` correctly stopped the server feeding a request-body `Settings`
override to the probe (it was an SSRF reachability oracle), but the button was never changed, so the server
now discards those settings and probes the role's **stored** configuration while the UI presents the result
as a test of what the operator just typed. Two consequences: (1) an admin editing a host sees a verdict about
the OLD host with no indication — a false pass is worse than no button; (2) on a fresh gateway nothing is
stored, so Test can never succeed during exactly the bootstrap flow AGENTS.md requires to keep working
("The admin UI must keep working in unconfigured mode"). By this document's own scale that is **High** — a
feature that silently does not work. Found by the orchestrator while verifying item 12; no worker filed it.
FIX — pick one, do not just delete the button: (a) take `C2`'s *other* documented option — gate `/test`
behind a stricter capability than plain admin and restore body-settings probing, which preserves the
...
`H35` **Info** `IContentStore.UpdateItemAsync` has no way to receive a previously-known revision/state
from the caller, which is why `JmapMailStore.CategoriesOfAsync` (`H10`, closed N/A) must re-fetch
current keywords from the server instead of being handed them — `SyncHandler` already knows the prior
categories from its own revision map (`AGENTS.md`'s sync model: mail revision includes `|kw1,kw2` sorted
category keywords) at the point it detects a client Change and calls `UpdateItemAsync`, but the interface
has no parameter to carry that down. FIX (large, Contracts-touching, not attempted here): add an
optional "known previous state" parameter to `IMailOperations.UpdateItemAsync` (or a capability
interface a store opts into) so a store that CAN use it (JMAP categories, and potentially similar
read-modify-write patterns elsewhere) skips its own fetch. Touches the plugin contract and every
backend's signature — worth doing only if a stronger motivating case than one Nit-level round trip
shows up. Found by the worker implementing item 23 while investigating `H10`. **Not yet assigned to an
item.**
