...
**12. SSRF, oracle & info disclosure** — ~~`C2`~~ ~~`C5`~~ ~~`E6`~~ ~~`E11`~~ **COMPLETE**
...
`C5` **Low** Portal `me`/`backends/meta` echo the admin-set backend username to a non-admin — `Api/PortalEndpoints.cs:48`. FIXED (item 12): the `me` handler now withholds a role's `userName` (returns null) unless the role/its effective provider opts at least one field into self-service (`HasSelfServiceSurface`, the same `SelfServiceEditable` gate the settings dictionary already uses) — e.g. carddav's Contacts role (zero self-service fields) no longer discloses an admin-bound service-account login, while caldav's Calendar role (CalendarAttachments/SendInvitations/SharedCollections opted in) still shows it. `backends/meta` was already values-free (schema only) and untouched. BEHAVIOUR CHANGE: `userName` is now `null` in the `me` response for any role with no self-service surface, even though the DB value is unchanged. **CORRECTED (post-item-12 repair):** the item-12 fix gated the read on the wrong surface — `SelfServiceEditable` governs the `settings` dictionary (connection knobs), but backend credentials (`userName`/`password`) are a separate, unconditional self-service surface (`PUT /user/api/backends/{role}` sets `UserName` for any role with no `SelfServiceEditable` check at all, per this file's own header comment and docs/webui.md). Gating the read broke the write/read round trip for a value the write side never restricted, without closing any real disclosure, and broke 2 of the 3 live integration failures the orchestrator bisected to item 12. `HasSelfServiceSurface` is removed and `userName` is echoed unconditionally again, as it was before item 12.
