...
`H2` **Med** CalDAV create always does a full pre-PUT enumeration (contradicts H13) — `Dav/DavStoreBase.cs:98`. FIXED (item 23): `CreateItemAsync` now builds `before` as a memoized `Func<Task<...>>` instead of an eagerly-awaited dictionary; `ResolveStoredHrefAsync` only invokes it where it's actually consulted — the `PathsEqual(hit.Href, putHref)` short-circuit means a well-behaved server (UID query returns the exact PUT href) never triggers the full enumeration at all. The listing-diff fallback (a UID-query-less or href-rewriting server) still fetches it, once, exactly as before. Proven red-first (`CardDavStoreTests.CreateItem_WhenUidQueryMatchesPutHref_SkipsTheFullEnumeration`, counting PROPFIND calls via a stub HTTP handler): 1 on unmodified code (the eager fetch), 0 after the fix.
