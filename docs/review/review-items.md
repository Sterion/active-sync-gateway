...
**12. SSRF, oracle & info disclosure** — ~~`C2`~~ `C5` ~~`E6`~~ ~~`E11`~~ **PARTIAL — C5 re-scoped and OPEN**
> `C5` was struck in item 12 and its fix then **reverted** during repair (`fec1cfe`), so the strike was a
> false record and has been removed by human decision. The finding is re-scoped below: the disclosure is
> real, but item 12's gate was the wrong shape — it keyed on the *settings* self-service surface, while
> credentials are unconditionally self-editable, which broke a legitimate write/read round trip. Do not
...
`C5` **Low — OPEN, re-scoped** Portal `me` echoes the admin-set backend username to a non-admin — `Api/PortalEndpoints.cs:48`. **RE-SCOPED after a failed item-12 attempt — read this whole entry before touching it.** THE DEFECT (unchanged): for a role whose credentials an admin configured (often a shared mail login or service account), the portal `me` response discloses that `userName` verbatim to the non-admin portal user. Being able to OVERWRITE a value does not entitle the caller to READ what an admin bound — those are different permissions. WHAT ITEM 12 GOT WRONG (do not repeat): it gated the echo on whether the role has any `SelfServiceEditable` **settings** field (`HasSelfServiceSurface`). That is the wrong surface — backend CREDENTIALS are a separate, unconditional self-service surface (`PUT /user/api/backends/{role}` sets `UserName` for any role with no `SelfServiceEditable` check, `PortalEndpoints.cs:233`), so the gate withheld a value the same caller may freely rewrite: it broke the write/read round trip (a user who set their own `userName` could no longer see it) and failed two live tests, without closing the disclosure in any way the caller could not undo. Reverted in `fec1cfe`. THE REAL CONSTRAINT: a correct fix must distinguish *the value an admin set* from *the value this user set*, and `AccountOptions` does not track per-field provenance today. FIX, in order of preference: (a) rule that the disclosure is acceptable precisely because the caller can already overwrite the field, and close this `N/A` with that reasoning recorded — a legitimate outcome, not a cop-out; or (b) add the provenance needed to withhold only admin-set credentials while still echoing the user's own. Do NOT re-introduce a settings-surface gate. `backends/meta` is values-free (schema only) and was never part of this.
