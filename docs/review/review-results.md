...
## Item 25 — Compose, move & folder handlers [LIVE]  ⚠ REQUIRED AN ORCHESTRATOR-INITIATED REPAIR
**Findings:** `F2` `F4` `F5` `F6`
**Commits:** `112c751` (F4) · `1255265` + `25de5e8` (F5) · `e85eb9c` (F6) · `cabebab` (F2, defective) ·
`7ee97c6` (**F2 repair**)
**Verification:** integrity items=32 assigned=132 dupes=0 encoding=0 ✓ · cursor → item 26 ✓ · strike with
each fix ✓ · build 0 warnings ✓ · unit **1164 passed, 0 skipped** (+14) ✓ · live **141 passed, 0
skipped** on a clean-volume Stalwart ✓

### ⚠ `F2` as first landed traded a duplicate send for SILENT MAIL LOSS
`cabebab` claimed a send durably on its own context **before** the irreversible action, and had **no
release path** — nothing ever rolled a claim back, and a claim recorded only that the send was
*attempted*. The very next statement on the draft-submit Add path is `await SubmitDraftAsync(...)` with
no try/catch. So on an ordinary transient failure (SMTP backend down, network blip, auth expiry):
the claim is already committed → the send throws → the round never commits, the SyncKey never advances →
the client resends under the same key → `TryClaimSendAsync` returns false → the handler returns
**Status 1 (success)**, records the ledger entry, and on the Change path removes the item from the
snapshot — **without ever sending**. The user's outgoing mail disappears and the client is told it was
sent. That is strictly worse than the duplicate `F2` set out to fix: a duplicate is visible and
recoverable, a false success is neither. All three claim sites shared the shape.
**Repair** (`7ee97c6`, scoped subagent): split *claimed* from *succeeded*. `SentCommandToken` gains a
`Completed` flag; `TryClaimAsync` returns `PerformSend` when there is no row **or** a row that was never
completed (crashed, in flight, or genuinely failed — indistinguishable, so retry), and `AlreadySent` only
once `Completed` is true; `MarkCompletedAsync` is called on its own durable context immediately after the
action succeeds. Because completion is only ever marked after success, a thrown exception naturally skips
it — no catch/release needed at the call sites. Migrations for **both** providers
(`AddSentCommandTokenCompleted`), snapshots regenerated by `dotnet ef`, not hand-edited.
**I re-established the red independently** rather than accepting the report (the worker had used a stash
to see it): reverting the four production files to `25de5e8` while keeping the new test, the repair's
test fails with `Assert.Single() Failure: The collection was empty` — the resend sending nothing, which
is the exact silent-loss symptom. `F2`'s original test (`...DoesNotSubmitTwice`) still passes, so both
properties hold at once, which was the acceptance criterion.
**Residual window, knowingly left:** a crash between the action succeeding and `MarkCompletedAsync`
landing still duplicates on resend. It cannot be closed without atomicity across SMTP and the local DB.
It shrinks `F2`'s original window (send → end of the whole round's commit) to one in-process DB round
trip, and it fails in the correct direction — a visible, recoverable duplicate rather than a silent loss.

**Other diffs read against the detail entries:**
- `F4` — `ResolveSourceAsync` memoized per handler instance (fresh per request, so safe). Red-first by
  counting `UserFolders` SELECTs through a new `DbCommandInterceptor`: 2 → 1.
- `F5` — the destination snapshot now stores the backend's **real** revision instead of the literal
  `"moved"`. Red-first (`"moved"` → `"101"`). **The live suite then caught a bug the fix itself
  introduced** — `FetchAsync` against an unopened IMAP folder, surfacing as MoveItems Status 5 instead of
  3 — fixed in `25de5e8`. That is the live suite earning its place twice over in one item.
- `F6` — a real parent change is now rejected with Status 3 instead of being silently ignored while
  reporting Status 1. One of the two remedies the finding offers. Red-first.
**⚠ BREAKING PLUGIN-CONTRACT CHANGE — the most significant of the programme so far:**
`F5` changed `IItemMoveOperations.MoveItemAsync` in `ActiveSync.Contracts` from `Task<string>` to
`Task<(string ItemKey, string Revision)>`. Unlike items 17–18 (type moves and a namespace rename), this
alters the signature of a method **plugins implement**: every out-of-repo plugin with an item-move
capability fails to compile. All four in-repo implementers were updated. `ContractVersion` is **still
1.0** — see item 17's note; that question is now materially sharper and needs a human decision.
**Notes:**
- **My resume message steered `F2`'s remedy and I should record that.** The worker's own judgment was to
  extend the AGENTS.md caveat, which the finding permits ("if not fixing, extend the AGENTS.md caveat").
  I asked for a genuine red-first reproduction, which pushed it to the finding's *other*, explicitly named
  option ("a real fix persists a send-dedup token before the send"). So the choice stayed inside the
  finding's terms — but a new table plus migrations on both providers is a large change for a Medium, and
  it would not have happened without the nudge. It also would not have needed a repair without the nudge.
- The worker's original run was killed mid-item by a server-side API error (529) after `F4`/`F5`; it was
  resumed from its own transcript with the tree state re-verified, and finished `F6` then `F2`.
- Findings were worked out of the listed order (F4, F5, F6, then F2). No stated sequencing constraint, so
  no consequence.
- **Disclosed coverage gap:** the occurrence-CANCEL path shares the identical guard but has no dedicated
  handler-level test (the harness has no ready helper for a pre-populated DAV href short-id) — verified by
  code review only. The worker disclosed this rather than letting it pass silently; it is the weakest
